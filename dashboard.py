...
if choose == "Sobre o App":
    ...
    st.write(
        ...
        com o objetivo de conduzir uma análise de potenciais clientes através da base de dados coletada pela empresa."""
    ...
    st.write("""A regra de associação é uma técnica poderosa no campo da mineração de dados e análise estatística, 
             especialmente útil para descobrir padrões e relações subjacentes em conjuntos de dados vastos e complexos. Essa técnica busca identificar associações frequentes entre os itens dentro de conjuntos de dados transacionais ou em outros tipos de dados tabulares."""
)
    ...
    st.write("""Nesta análise, foi utilizado o algoritmo Apriori. O algoritmo Apriori é usado para descobrir padrões de associação em grandes conjuntos de dados. Ele opera em três etapas principais:

    - Identificação de conjuntos de itens frequentes.
    - Geração de todas as regras de associação possíveis a partir desses conjuntos.
    - Seleção das regras mais significativas com base em critérios de suporte e confiança.

O resultado final é um conjunto de regras de associação que descrevem padrões importantes nos dados, úteis para recomendações, segmentação de clientes e otimização de processos de negócios. """
)
...
if choose == "Sobre os Dados":
    ...
    st.write("""As bases de dados foram segmentadas entre informações de Pessoa Física e Pessoa Jurídica. 
             A análise foi conduzida de maneira distinta, alinhada aos interesses específicos de cada tipo de cliente, visando aprimorar a abordagem na análise de potenciais clientes.
             """)
